...
class Task(Base):
    ...
    save_images = Column(Boolean, default=False, comment="是否保存图像结果")
    analysis_interval = Column(Integer, default=1, comment="分析间隔(秒)")
    specific_node_id = Column(Integer, nullable=True, comment="指定节点ID，不区分HTTP和MQTT节点")
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class SubTask(Base):
    ...
    name = Column(String(200), nullable=True, comment="子任务名称，通常为摄像头名称+算法名称")
